...
coeffK3 = Coefficient[nabRaw /. {polk1->0}, polk3];
Print["k1-coeff LC: ", LeafCount[coeffK1], "   k3-coeff LC: ", LeafCount[coeffK3]];

(* ============================================================================
   THE TEST THAT SETTLES IT.
   This object -- the massive non-abelian radiator -- is EXTERNALLY VALIDATED:
   18 digits against Hoeche's closed form in the massless limit, and 11-13 digits
   against the BCM soft current at physical kinematics.  It is not in doubt.
   For a SCALAR the polarisation vector appears as a genuine Pair, so the Ward
   identity has content:  writing  M^mu eps_mu = A (k1.eps) + B (k2.eps) + C (k3.eps),
   transversality of the emitted gluon (k2^2 = 0) requires
        ((t-m^2)/2) A + ((u-m^2)/2) C = 0 ,
   which is precisely the statement that the radiator carries the dipole structure
        p_i/(t-m^2) - p_k/(u-m^2) .
   If this FAILS for an amplitude we know is right, then the premise is wrong -- these
   radiators are gauge-dependent building blocks by construction, gauge invariance
   holding only for Hoeche's specific assembly -- and the Deliverable-B "Ward failure"
   is not a bug at all.  If it PASSES, the premise is sound and the fermionic set
   really is defective.                                                           *)
ward = Simplify[ ((t - mq^2)/2) coeffK1 + ((u - mq^2)/2) coeffK3 ];
Print[""];
Print["=== WARD RESIDUAL of the VALIDATED non-abelian radiator ==="];
Print["  symbolic zero?  ", PossibleZeroQ[ward] || Simplify[ward] === 0];
Do[Module[{v = Simplify[ward /. pt]},
   Print["  at ", pt, "  ->  ", Chop[N[v, 10], 10^-10]]],
 {pt, {{s->-3, t->-5, u->-2, mq->1}, {s->-7, t->-2, u->-11, mq->1},
       {s->-13, t->-3, u->-5, mq->2}}}];
Print[""];
Print["  (for reference, k1-coeff at the first point: ",
      Chop[N[Simplify[coeffK1 /. {s->-3,t->-5,u->-2,mq->1}],8],10^-10], ")"];
