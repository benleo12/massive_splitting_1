...
\centering
\begin{tabular}{ccc}
\begin{tikzpicture}[baseline=(current bounding box.center),scale=0.82,every node/.style={transform shape}]
\begin{feynman}
  \vertex [blobv] (H) at (0,0) {};
...
\end{feynman}\end{tikzpicture}
\\[1mm]
\footnotesize (a) box & \footnotesize (b) factorizable & \footnotesize (c) seagull
\end{tabular}
...
production vertex. A gauge-invariant set is formed by the box (a), the factorizable
hard-vertex correction (b) and the scalar seagull (c); crossing-symmetric partners
are understood. The single colour decomposition $2\,\Top{i}^a\Top{i}^b
=if^{abc}\Top{i}^c+\{\Top{}^a,\Top{}^b\}_i$ splits their sum into the abelian
...
colour-charged spectator $l\neq i,k$ does not arise, as the source produces only
the $i,k$ dipole.}
\label{fig:boxes}
\end{figure}
...
Fig.~\ref{fig:boxes}. A gauge-invariant set is formed by the box (a), the factorizable hard-vertex
correction (b) and the scalar seagull (c).
