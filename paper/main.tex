...
gluon from a pair of \emph{massive} colour charges, the massive counterpart of
...
soft--collinear overlap to disentangle and the corresponding NNLO subtraction
requires no phase-space sectorization.}
...
\emph{scalar dipole radiators} and \emph{pure splitting remainders} was
...
Our calculation is \emph{fully numerical}. The required master integrals are
...
amplitude is \emph{not} gauge invariant: replacing $\eps^\mu(q)\to q^\mu$ leaves
\begin{equation}
...
\begin{feynman}
  \vertex [blobv] (v5) at (0,0) {};
  \vertex [dot] (v6) at (1.25,0.95) {};
  \vertex [dot] (v8) at (1.25,-0.95) {};
  \vertex [dot] (v7) at (2.45,0) {};
  \vertex (oi) at (2.40,1.85);
  \vertex (ok) at (2.40,-1.85);
  \vertex (og) at (3.75,0);
  \diagram*{
    (v5) -- [charged scalar] (v6),
    (v5) -- [charged scalar] (v8),
    (v6) -- [gluon] (v7),
    (v8) -- [gluon] (v7),
    (v6) -- [charged scalar] (oi),
    (v8) -- [charged scalar] (ok),
    (v7) -- [gluon] (og),
  };
\end{feynman}
\end{tikzpicture}
...
\begin{feynman}
  \vertex [blobv] (v5) at (0,0) {};
  \vertex [dot] (v6) at (1.25,0.95) {};
  \vertex [dot] (v7) at (1.25,-0.95) {};
  \vertex [dot] (v8) at (2.45,0) {};
  \vertex (oi) at (2.40,1.85);
  \vertex (og) at (2.40,-1.85);
  \vertex (ok) at (3.75,0);
  \diagram*{
    (v5) -- [charged scalar] (v6),
    (v5) -- [charged scalar] (v7),
    (v6) -- [gluon] (v8),
    (v7) -- [charged scalar] (v8),
    (v6) -- [charged scalar] (oi),
    (v7) -- [gluon] (og),
    (v8) -- [charged scalar] (ok),
  };
...
\begin{feynman}
  \vertex [blobv] (v5) at (0,0) {};
  \vertex [dot] (v8) at (1.15,0.85) {};
  \vertex [dot] (v7) at (1.15,-0.85) {};
  \vertex [dot] (v6) at (2.45,1.35) {};
  \vertex (oi) at (3.60,1.85);
  \vertex (og) at (3.60,0.75);
  \vertex (ok) at (2.45,-1.60);
  \diagram*{
    (v5) -- [charged scalar] (v8),
    (v5) -- [charged scalar] (v7),
    (v7) -- [gluon] (v8),
    (v8) -- [charged scalar] (v6),
    (v6) -- [charged scalar] (oi),
    (v6) -- [gluon] (og),
    (v7) -- [charged scalar] (ok),
  };
\end{feynman}
\end{tikzpicture}
&
...
\begin{feynman}
  \vertex [blobv] (v5) at (0,0) {};
  \vertex [dot] (v6) at (1.15,0.85) {};
  \vertex [dot] (v8) at (1.15,-0.85) {};
  \vertex [dot] (v7) at (2.45,-1.35) {};
  \vertex (oi) at (2.45,1.60);
  \vertex (og) at (3.60,-0.75);
  \vertex (ok) at (3.60,-1.85);
  \diagram*{
    (v5) -- [charged scalar] (v6),
    (v5) -- [charged scalar] (v8),
    (v6) -- [gluon] (v8),
    (v8) -- [charged scalar] (v7),
    (v7) -- [gluon] (og),
    (v7) -- [charged scalar] (ok),
    (v6) -- [charged scalar] (oi),
  };
...
\end{tikzpicture}
\\[1mm]
\footnotesize (d) & \footnotesize (e)
\end{tabular}
\caption{The five one-loop topologies that make up the massive \emph{non-abelian}
scalar dipole radiator computed in this work, labelled by their index in the
generated diagram set. Panel (a) is the box in which the emitted gluon is
attached at the triple-gluon vertex; (b) and (c) are the two orientations of the
box in which it is emitted from a scalar line inside the loop; (d) and (e) are
the two orientations of the triangle correction to the hard production vertex
with the gluon emitted from an external leg. Diagrams (b), (c) and (d), (e) are
combined pairwise with the colour projection
$T^aT^bT^c\to\tfrac{i}{2}f^{bcd}T^aT^d$ that isolates the $f^{abc}$ structure.
All internal scalar lines carry the mass $m$; the loop momentum flows around the
closed circuit in each panel.}
\label{fig:boxes}
\end{figure}

The genuinely one-loop, non-factorizable structure resides in the box-type
diagrams, Fig.~14 of Ref.~\cite{Campbell:2025}. A gauge-invariant set is obtained
by combining the box and seagull topologies with two factorizable members: a
correction dressing the hard production vertex, and a diagram in which the gluon
is emitted from a colour-charged spectator $l\neq i,k$. The topologies we
actually evaluate in the massive case are shown in Fig.~\ref{fig:boxes}; for a
dipole there is no spectator, so the second factorizable member is absent and its
role is played by the triangle corrections of Fig.~\ref{fig:boxes}(d), (e). The
...
\begin{equation}
  ...
  \label{eq:hardvertex}
\end{equation}
whose $t\to0$ limit is $2Q^2\Ihat_3^{1m}(Q^2)+\Ihat_2(Q^2)$, the spectator-emission
case. Using Eq.~\eqref{eq:colour-identity} the factorization counterterm splits
into
\begin{align}
...
the sum \emph{before} the counterterm \eqref{eq:ct-nab} is removed. Writing that
unsubtracted sum as
\begin{equation}
\begin{split}
...
\emph{tree} squared amplitude of the scalar dipole is, in the singlet projection,
\begin{equation}
...
invariants $(s,t,u)$ and the masses $(m_i^2,m_k^2)$ declared as \emph{real
...
Eq.~\eqref{eq:Bnab} gives a ratio that is \emph{independent of the kinematics},
\begin{equation}
...
\emph{as an exact algebraic identity} in the reduced amplitude: the combination
...
\begin{equation}
  ...
  \label{eq:bcmmap}
\end{equation}
and construct genuinely soft, \emph{physical} configurations by fixing the two
...
\emph{entirely on the abelian sector}. Decomposing the complete radiator by
...
abelian double pole, which in the massless theory arises from the \emph{overlap}
...
soft--collinear \emph{overlap}; it is not needed for a pure soft singularity.
...
collinear singularity produces a \emph{constant} increment per fixed ratio of
...
normalisation independent, is that the abelian double pole \emph{vanishes} while
the non-abelian one does not.

Since the scalar dipole radiator captures precisely the soft structure, and the
soft structure is now the \emph{entire} singular structure, the massive one-loop
splitting amplitude \emph{is} its scalar dipole radiator up to a finite,
...
%=====================================================================
\section{Towards the massive splitting function}
\label{sec:splitting}
%=====================================================================
The radiator computed above is one of the two ingredients of a splitting
amplitude. In the decomposition of Ref.~\cite{Campbell:2025} the quark--gluon
vertex is written as a scalar piece, a magnetic piece and a term proportional to
...
\end{equation}
so that the one-loop splitting amplitude for a massive quark is the scalar
dipole radiator of Sec.~\ref{sec:massless} plus a \emph{magnetic remainder}. In
terms of spin- and colour-summed interferences with the tree amplitude the
remainder is the difference
\begin{equation}
  \mathcal{R}^{\rm mag}
  =\frac{2\,\mathrm{Re}\big\langle \mathcal{M}^{(0)}_{q}\big|\mathcal{M}^{(1)}_{q}\big\rangle}
        {\big|\mathcal{M}^{(0)}_{q}\big|^{2}}
  -\frac{2\,\mathrm{Re}\big\langle \mathcal{M}^{(0)}_{\tilde q}\big|\mathcal{M}^{(1)}_{\tilde q}\big\rangle}
        {\big|\mathcal{M}^{(0)}_{\tilde q}\big|^{2}}\;,
  \label{eq:magnetic-remainder}
\end{equation}
evaluated with identical loop content on the two sides: gluon, ghost and closed
\emph{quark} loops on both, no gluino and no closed \emph{squark} loops. We have
verified that the six closed-squark-loop diagrams that would otherwise appear on
the scalar side form a separately gauge-invariant subset, so removing them to
match the quark side is legitimate.

Both interferences must be taken in the colour-singlet projection of the source,
Eq.~\eqref{eq:ward-residual}; with the source colour indices left open neither
amplitude is gauge invariant and the difference \eqref{eq:magnetic-remainder} is
not meaningful. The fermionic and scalar radiator amplitudes and their master
bases are in hand ($22$ and $19$ masters respectively, all validated as in
Sec.~\ref{sec:validation}), and the interference is assembled with the
polarisation sum $\sum\eps^\mu\eps^{*\nu}\to-g^{\mu\nu}$, which is licensed by
the Ward identity once the singlet projection is imposed.

We emphasise that Eq.~\eqref{eq:magnetic-remainder} is \emph{not} evaluated in
the present paper: what is established here is the scalar dipole radiator, its
validation, and the structural statement about sectorization, all of which are
independent of the magnetic piece. The assembled massive splitting functions,
for which \eqref{eq:magnetic-remainder} is the missing ingredient, will be
reported separately.
...
singular structure and the massive NNLO subtraction needs no sectorization.
Together with the fully numerical assembly of the real-emission integrals, this
indicates that a FONLL-matched, fully numerical NNLO calculation of heavy-quark
production for FCC-ee is within reach. The remaining ingredient, the massive
magnetic (spin) remainder that completes the quark and gluon splitting functions,
...
is defined in Eq.~\eqref{eq:gram}. The reduction leaves \emph{no} rational
